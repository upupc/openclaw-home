...
1. 先要求用户提供 `config` 文件路径。
2. 如果用户还没有配置文件，明确提示至少需要这些 JSON 字段：
   - `empIds` 
   ...
   - `git.token`
   - `git.username`
   - 可选：`architect.endpoint`，默认 `https://pre-architect.alibaba-inc.com/api/queryRecentMostCommittedRepos`
3. 再要求用户提供要评估的工号，支持多个，并写入配置文件的 `empIds` 数组。
4. 运行员工与仓库查询脚本：
